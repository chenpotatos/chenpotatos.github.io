...
dp.sh.RegexLib = {
	...
	SingleLineCComments: new RegExp("//.*$", "gm"),     //这里有个bug:会把 http:// 网址当做注释
...
dp.sh.Highlighter.prototype.SwitchToList = function() {
	...
	for (var i = 0,
	lineIndex = this.firstLine; i < _43.length - 1; i++, lineIndex++) {
...
dp.sh.Brushes.Python = function() {
	...
	{
		regex: new RegExp(this.GetKeywords(magicmethods), 'gm'),
		css: 'magicmethods'
	},
	{
		regex: new RegExp(this.GetKeywords(exceptions), 'gm'),
		css: 'exceptions'
	},
	{
	...
	{
		regex: new RegExp(this.GetKeywords(commonlibs), 'gm'),
		css: 'commonlibs'
	}];
	this.CssClass = 'dp-py';
};
...
//不高亮
dp.sh.Brushes.dpnull = function() {
	this.CssClass = 'dp-no';
};
dp.sh.Brushes.dpnull.prototype = new dp.sh.Highlighter();
dp.sh.Brushes.dpnull.Aliases = ['null'];
dp.sh.Brushes.dpnull.prototype.ProcessRegexList = function() {
	function push(array, value) {
		array[array.length] = value;
	};
	var index = 0;
...
dp.sh.Brushes.Perl = function() {
	...
	{
		regex: new RegExp(this.GetKeywords(declarations), 'gm'),
		css: 'declarations'
	}];
...
dp.sh.Brushes.Groovy = function() {
	...
	{
		regex: new RegExp(this.GetKeywords(modifiers), 'gm'),
		css: 'modifier'
	},
	{
