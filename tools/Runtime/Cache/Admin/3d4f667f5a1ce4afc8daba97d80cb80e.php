            ...
            <ul class="navbar-nav">
              ...
              <li class="nav-item dropdown active">
                ...
                <ul class="dropdown-menu multilevel scale-up-left">
                	...
                	<li class="nav-item"><a class="nav-link active" href="<?php echo U('tools/classe');?>">分类管理</a></li> 
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="box">
            ...
            <div class="box-header with-border">
            <a href="<?php echo U('tools/classadd');?>" class="btn btn-outline btn-rounded btn-info">新增一个分类</a>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
				  ...
				  <table id="example1" class="table table-bordered table-striped">
					<thead>
						<tr>
							...
							<th><span class="badge badge-info">分类名称</span></th>
							<th><span class="badge badge-warning">分类标识</span></th>
							<th><span class="badge badge-danger">操作</span></th>
						</tr>
					</thead>
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$classe): $mod = ($i % 2 );++$i;?><tr>
							<td><?php echo ($classe["id"]); ?></td>
							<td><?php echo ($classe["classname"]); ?></td>
							<td><?php echo ($classe["classid"]); ?></td>
							 <td>
								<a href="<?php echo U('tools/classedit');?>?cid=<?php echo ($classe["id"]); ?>" class="btn btn-outline btn-rounded btn-primary">编辑</a>
								<a href="javascript:void(0)" onClick="ty_del(this,'<?php echo ($classe["id"]); ?>')"  class="btn btn-outline btn-rounded btn-danger">删除</a>
							</td>
...
<!-- ./wrapper -->
	...
	<script type="text/javascript">
		function ty_del(obj, id) {
            var id = id;
            layer.confirm('确定要删除分类？注意此操作不可逆！该分类下所有的工具列表将会删除，文件不会删除', function (index) {

                $.ajax({
                    type: 'POST',
                    url: '<?php echo U('tools/classedel');?>',
                    dataType: 'json',
                    data: { id: id },
