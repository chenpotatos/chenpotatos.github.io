(function ( w, d, PKAE ) {
	...
	function _topbarConfig ( app, ui ) {
		return [
								  ...
								  buttons:[
									{
										...
										callback: function( q ) {
											...
											{
												OneUp ('无效的音频Url链接', 1100);
											}
											// -
										}
									}
								  ],
								  body:'<label for="k00">请输入音频地址</label>' + 
									'<input style="min-width:250px" placeholder="请输入地址" class="pk_txt" type="text" id="k00" />',
								  setup:function( q ) {
						...
						action: function ( e ) {
							...
							new PKSimpleModal ({
								...
								body:'<p>请选择来源...</p>' +
									...
									'<input style="min-width:250px" placeholder="文件名（可选）" maxlength="100" ' +
						...
						setup: function ( obj ) {
							...
							app.listenFor ('DidStoreDB', function ( obj, e ) {
									...
									var txt = '<div style="padding:2px 0">id: ' + name + '</div>'+
										'<div style="padding:2px 0"><span>持续: ' + obj.durr + 's</span>'+
										'&nbsp;&nbsp;&nbsp;'+
										'<span>变化: ' + (obj.chans === 1 ? 'mono' : 'stereo') + '</span></div>'+
								...
								fls.ListSessions(function( ret ) {
									...
									else
									{
										for (var i = 0; i < ret.length; ++i)
										{
											...
											'<span style="text-align:right;float:right" class="pk_lcls"><i class="pk_i">持续:</i>' + duration + 's</span></div><div>' +
			...
			},
			{
				...
				children:[
					{
						name: '撤消 <span class="pk_shrtct">Shft+Z</span>',
						clss: 'pk_inact',
						action: function () {
						...
						setup: function ( obj ) {
							app.listenFor ('DidStateChange', function ( undo_states, redo_states ) {
								...
								{
									obj.innerHTML = '撤消 <span class="pk_shrtct">Shft+Z</span>';
									obj.classList.add ('pk_inact');
								}
								else
								{
									obj.innerHTML = '撤消&nbsp;<i style="pointer-events:none">' + undo_states[undo_states.length - 1].desc + '</i><span class="pk_shrtct">Shft+Z</span>';
									obj.classList.remove ('pk_inact');
								}
							});
						}
					...
					
					{
						...
						setup: function ( obj ) {
							app.listenFor('DidStateChange', function ( undo_states, redo_states ) {
								...
								{
									obj.innerHTML = '重置 <span class="pk_shrtct">Shft+Y</span>';
									obj.classList.add ('pk_inact');
								}
								else
								{
									obj.innerHTML = '重置&nbsp;<i style="pointer-events:none">' + redo_states[0].desc  + '</i><span class="pk_shrtct">Shft+Y</span>';
									obj.classList.remove ('pk_inact');
								}
							});
						}
	...
	function _makeUIToolbar (UI) {
		...
		btn_play.innerHTML = '<span>开始 (Space)</span>';
		...
		btn_back_jump.innerHTML = '<span>查询 (left arrow)</span>';
		...
		btn_front_jump.innerHTML = '<span>查询 (right arrow)</span>';
		...
		btn_back_total.innerHTML = '<span>开始查询 (Shift + left arrow)</span>';
		...
		btn_front_total.innerHTML = '<span>结束查询 (Shift + right arrow)</span>';
		...
		btn_rec.innerHTML = '<span>记录 (R)</span>';
		...
		btn_clear_selection.innerHTML = '<span>清空选项 (Q key)</span>';
